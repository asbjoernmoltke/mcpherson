...
- [x] ⚠️ **Camera-cold interlock on turbo-stop/standby** (2026-06-16): because turbo-stop auto-vents, `SafetyManager.assert_can_stop_pumping()` refuses turbo-off **and** turbo-standby while the camera is cold (`CameraController.is_cold` = cooler on and sensor below the 10 °C warm target) — the worker pump slots call it, so a refusal pops up "Operation refused". Warm the camera before stopping the turbo. (Backing-off is not gated: by then the turbo is stopped and already vented.)
- [x] ⚠️ **Pump fault/alert surfacing** (2026-06-16): every TIC `?V` reply ends in `;<alert id>;<priority>`; `EdwardsTIC.read_alerts()` decodes the manual §1.7.3 alert table for the turbo/backing/gauge (alert = 2nd-to-last field). `SafetyManager.check_pump_health()` (run each poll) raises the banner alarm for any active alert **and** for "turbo spinning with no backing pump" (lost exhaust); the GUI Vacuum panel has a dedicated **Alerts** field (red when active). Tested offline via `DummyVacuum.set_alerts`.
